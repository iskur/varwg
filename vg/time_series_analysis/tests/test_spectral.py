...
class Test(npt.TestCase):
    ...
    def test_1d_simulate(self):
        cov_model_ar1 = lambda lag: old_div(self.rho ** lag, (1 - self.rho ** 2))
        n_lags = 10
        ...
        for data in data_sim:
            cov_act = np.array([time_series.auto_cov(data, lag)
                                for lag in range(n_lags)])
            npt.assert_almost_equal(cov_act, cov_exp, decimal=1)
        data_sim = spec.sim
        cov_act = np.array([time_series.auto_cov(data_sim, lag)
                            for lag in range(n_lags)])
        npt.assert_almost_equal(cov_act, cov_exp, decimal=1)
    ...
    def test_spectral_2d_covdata(self):
        ...
        T = 7500
        data_obs = models.VAR_LS_sim(test_models.B_test,
                                     test_models.sigma_u_test, T)
        ...
        for var_i in range(data_obs.shape[0]):
            ac_obs = [time_series.auto_cov(data_obs[var_i], lag)
                      for lag in range(7)]
            ac_sim = [time_series.auto_cov(data_sim[var_i], lag)
                      for lag in range(7)]
