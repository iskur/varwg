...
class Test(TestCase):
    ...
    def setUp(self):
        self.verbose = False
        self.data = np.array(
            [47, 64, 23, 71, 38, 64, 55, 41, 59, 48, 71, 35, 57, 40, 58, 44,
             80, 55, 37, 74, 51, 57, 50, 60, 45, 57, 50, 45, 25, 59, 50, 71,
             56, 74, 50, 58, 45, 54, 36, 54, 48, 55, 45, 57, 50, 62, 44, 64,
             43, 52, 38, 59, 55, 41, 53, 49, 34, 35, 54, 45, 68, 38, 50, 60,
             39, 59, 40, 57, 54, 23],
            dtype=float)
        self.sim = self.gen_time_series(rho=.9)
    ...
    def test_autocorr_ar(self):
        ...
        assert_almost_equal(.9, phi, decimal=2)

    def test_autocorr_ar_multi(self):
        """compare calculated autocorr on series with known autocorr 2dim."""
        phi = time_series.auto_corr([self.sim, self.gen_time_series(.6)], 1)
        assert_almost_equal([.9, .6], phi, decimal=2)
    ...
    def test_autocorr_ar_nans_multi(self):
        ...
        2dim.
        """
        sim = np.copy([self.sim, self.gen_time_series(.6)])
    ...
    def test_autocorr_box1(self):
        ...
        assert_almost_equal(phi, -.79, decimal=2)

    def test_autocorr_box2(self):
        """see p.31f Time Series Analysis Box Jenkins"""
        rk = np.array([-.39, .30, -.17, .07, -.10, -.05, .04, -.04, -.00, .01,
                       .11, -.07, .15, .04, -.01])
        phis = np.array([time_series.auto_corr(self.data, k)
                         for k in range(1, 16)])
        if self.verbose:
