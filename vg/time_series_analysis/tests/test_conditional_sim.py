...
from vg.time_series_analysis import (models, time_series,
                                     distributions)
...
class Test(npt.TestCase):
    ...
    def setUp(self):
        ...
        self.scale = 5
        self.speed = 5
        self.cov_model = lambda h: 5 * np.exp(old_div(-h, 5.))
        ...
        cond_eq_u_vals = np.array([-.5 * self.speed, -self.speed,
                                   self.speed, .75 * self.speed])
        ...
        cond_eq_v_vals = np.array([self.speed, -.75 * self.speed,
                                   .5 * self.speed, -self.speed])
        ...
        self.cond_mean_u_ii = [[list(range(self.domainshape[0])),
                                self.domainshape[0] * [0]]]
        self.cond_mean_u_vals = [5]

        self._uus, self._vvs = None, None
        T_agg = 20
        self.data_obs = models.VAR_LS_sim(test_models.B_test,
                                          test_models.sigma_u_test, T_agg)
        ...
        self.cov = np.cov(self.data_obs[:,
                                        np.all(np.isfinite(self.data_obs),
                                               axis=0)])
    ...
    def test_simulate(self):
        ...
        data_sim = csim.simulate(self.cov_model_ar1, T,
                                 cond_eq=(cond_eq_ii,
                                          cond_vals),
    ...
    def test_disaggregate_2d(self):
        ...
        pre = time.time()
        data_sim2 = dis2(self.data_obs, autocovs,
                         disagg_len=disagg_len, cov=cov, thresh=.01,
                         verbose=self.verbose)
        if self.verbose:
            print("Seconds of chunked version: ", time.time() - pre)
        means2 = np.mean(data_sim2.reshape(K, -1, disagg_len), axis=-1)
        npt.assert_almost_equal(means2[~nan_mask],
                                self.data_obs[~nan_mask], decimal=5)
        if self.verbose:
            pre = time.time()
        data_sim = csim.disaggregate(self.data_obs, autocovs,
                                     disagg_len=disagg_len, cov=cov,
                                     verbose=self.verbose)
        if self.verbose:
            print("Seconds in continuous version: ", time.time() - pre)
        means = np.mean(data_sim.reshape(K, -1, disagg_len), axis=-1)
        npt.assert_almost_equal(means[~nan_mask],
                                self.data_obs[~nan_mask])
        # npt.assert_almost_equal(np.cov(data_sim), cov)

        # did we change the covariance structure with the piecewise disag?
        lags = list(range(7))
        autocovs1 = np.array([time_series.auto_corr(data_sim, lag)
                              for lag in lags]).T
        autocovs2 = np.array([time_series.auto_corr(data_sim2, lag)
                              for lag in lags]).T
        ...
        # an whatnot
        npt.assert_allclose(autocovs2, autocovs1, rtol=.1, atol=.25)

    def test_disaggregate_2d_nonlin(self):
        """Disaggregate with a back-transformation."""
        np.random.seed(0)
        dist = distributions.expon(1, old_div(1., 15.))

        def trans(values, *args):
            return np.array([dist.ppf(distributions.norm.cdf(row))
                             for row in values])

        disagg_len = 5
        data_sim = csim.disaggregate_piecewice(self.data_obs,
                                               autocovs,
        ...
        means = np.mean(trans(data_sim.reshape(K, -1, disagg_len)),
                        axis=-1)
        ...
        npt.assert_almost_equal(means[~nan_mask], data_trans[~nan_mask],
                                decimal=0)
    ...
    def test_simulate_2d(self):
        ...
        cond_eq_ii = [len_conds * [0],
                      cond_eq_ii]
        cond_vals = len_conds * [5]
        time_before = time.time()
        data_sim = csim.simulate(autocovs, T, self.cov,
                                 cond_eq=(cond_eq_ii,
                                          cond_vals),
                                 verbose=self.verbose)
        if self.verbose:
        ...
        # npt.assert_almost_equal(np.cov(data_sim), cov)
        time_before = time.time()
        sim = csim.SimulateMulti(T, autocovs, self.cov, pool_size=320,
                                 verbose=self.verbose)
    ...
    def test_simulate_2d_mean(self):
        """Test the mean conditions of simulate."""
        T = 500
        cond_ii = ((10 * [0], list(range(210, 220))),
                   (20 * [0], list(range(220, 240))))
        cond_vals = (-3., 3)
        data_sim = csim.simulate(autocovs, T, cov,
