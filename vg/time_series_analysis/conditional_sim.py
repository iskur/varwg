...
def simulate(
    ...
    cond_eq_vals, cond_mean_vals = list(
        map(np.atleast_2d, (cond_eq_vals, cond_mean_vals))
    )
    ...
    while a_norm > 1.0:
        ...
        if cond_eq is not None:
            ...
            y_cond_values = np.array(
                [realization[cond_eq_ii] for realization in realizations]
            ).T
        ...
        if cond_mean is not None:
            ...
            y_cond_values = np.array(
                [
                    [rea[ij].mean() for rea in realizations]
                    for ij in cond_mean_ii
                ]
            )
            y_cond_values = y_cond_values.reshape(
                cond_mean_vals.shape[1], n_realizations
            )
        ...
        if verbose:
            print(
                "n_conditions: %d, n_realizations: %d, a_norm = %.3f"
    ...
    t = optimize.brentq(root_func, 0.0, 1.0)
...
class SimulateMulti(object):
    def __init__(
        self, T, autocovs, cov, pool_size=None, verbose=False, sum_vars=None
    ):
        ...
        self.conds_values_names = tuple(
            [name % "values" for name in self.conds_names]
        )
        self.conds_ij_names = tuple([name % "ij" for name in self.conds_names])
        self._T_original = T
        # for T property
        self._T = T
        self.reset()
        ...
        self.spec = spectral.MultiSpectral(
            autocovs, cov, T, pool_size=pool_size, verbose=verbose
        )
    ...
    def cond_trans_mean(self, ij, values, trans, t_kwds=None):
        ...
        if np.ndim(self.conds_trans_mean_values) == 1:
            self.conds_trans_mean_values = self.conds_trans_mean_values[
                :, None
            ]
    ...
    def run(self, inner_thresh=0.15):
        ...
        while norm_compl < (1.0 - norm_inner):
            ...
            idd = -1.0 * np.identity(A.shape[1] - n_conditions)
        ...
        tt = np.sqrt((1.0 - norm_inner) / norm_compl)
        return interpolator + np.tensordot(tt * xx, realizations_compl, axes=1)
    ...
    def _fit_transformed_mean(self, interim):
        ...
        targets = np.where(
            np.isfinite(targets), self.mean_trans(targets, doys), np.nan
        )
        agg_funcs = [
            self.agg_funcs[row_ii[0]] for row_ii, _ in conds_trans_mean_ij
        ]
        mean_targets = np.array(
            [
                agg(targets[list(map(list, cond_ij))])
                for agg, cond_ij in zip(agg_funcs, conds_trans_mean_ij)
            ]
        )
        ...
        def squared_error(alpha):
            ...
            norm_compl = np.dot(xx, xx)
            tt = np.sqrt((1.0 - interim.norm_inner) / norm_compl)
            sim = interim.interpolator + np.tensordot(
                tt * xx, interim.realizations_compl, axes=1
            )
            sim_trans = self.mean_trans(sim, doys)
            sim_trans_mean = np.array(
                [
                    agg(sim_trans[list(map(list, cond_ij))])
                    for agg, cond_ij in zip(agg_funcs, conds_trans_mean_ij)
                ]
            )
            ...
            return np.sum(
                (z_trans(sim_trans_mean) - z_trans(mean_targets)) ** 2
            )
        ...
        result = optimize.minimize(
            ...
            options=dict(disp=self.verbose, gtol=tol),
        )
        alpha = result.x
        rmse = np.sqrt(result.fun / max(1, len(conds_trans_mean_ij)))
        if rmse > 0.5:
            n_throwaway = int(len(interim.realizations_compl) / 3)
            realizations_compl = np.concatenate(
                (
                    interim.realizations_compl[n_throwaway:],
...
def disaggregate(
    data, autocov, disagg_len, cov=None, trans=None, t_kwds=None, **kwds
):
    ...
    **kwds : optional
    ...
    if trans is None:
        ...
        sim.cond_eq([list(range(K)), K * [0]], data[:, 0][None, :])
        sim.cond_trans_mean(cond_ii, cond_vals, trans, t_kwds)
    return sim.run()


def disaggregate_piecewice(
    data,
    autocov,
    disagg_len,
    cov=None,
    ...
    data_sim = np.empty(
        (T // (chunk_len - n_overlap) + 1, K, chunk_len - n_overlap)
    )
    ...
    if K == 1:
        ...
        cond_mean_ii = np.arange(n_overlap, chunk_len).reshape(-1, disagg_len)
    else:
        cond_eq_ii = (
            tuple(np.arange(K).repeat(n_overlap)),
            tuple(K * list(range(n_overlap))),
        )
        cond_mean_ii = tuple(
            [
                (disagg_len * [k], list(range(t, t + disagg_len)))
                for k in range(K)
                for t in range(n_overlap, chunk_len, disagg_len)
            ]
        )
    ...
    if "doys" in t_kwds:
        ...
        doys_chunked = [
            doys[
                t * disagg_len
                - (n_overlap / 2 if t > 0 else 0) : disagg_len
                * (t + chunk_len_agg)
    ...
    if verbose:
        print(
            "corr_len: %d, chunk_len: %d, n_overlap: %d, n_chunks: %d"
            % (corr_len, chunk_len, n_overlap, n_chunks)
        )

    data_sim[0] = disaggregate(
        data_chunked[0],
        acs,
        disagg_len,
        cov,
    ...
    if cov is not None:
        sim = SimulateMulti(
            chunk_len, acs, cov, pool_size=pool_size, verbose=verbose
        )
    for i in tqdm(range(1, n_chunks), disable=(not verbose)):
        ...
        if "doys" in kwds and kwds["doys"] is not None:
            if np.array(cond_mean_ii)[:, 1].max() > len(doys_chunked[i]):
                rest_len = (
                    data_chunked[i].shape[1] * disagg_len + n_overlap / 2
                )
                cols = [
                    list(range(t, min(t + disagg_len, rest_len)))
        ...
        if cov is None:
            ...
            if trans is None:
                ...
                if "doys" in t_kwds:
                    ...
                    kwds = {}
                sim.cond_trans_mean(
                    cond_mean_ii, cond_mean_vals, trans, t_kwds=kwds
                )
