...
class SeasonalDist6d(SeasonalDist):
    def __init__(self, *args, **kwds):
        super(SeasonalDist6d, self).__init__(*args, **kwds)
        self.n_trig_pars = 6
        self.pre_start = [1, 1, 0]
    ...
    def trig2pars(self, trig_pars, _T=None):
        ...
        try:
            # funny how the obvious thing looks so complicated
            a, b_0, phi, b2, per, phi2 = \
                np.array(trig_pars).reshape(-1, 6).T[:, :, np.newaxis]
            # allow only integer multiples of 1/year frequency
            _T2 = _T * 365 / per.round().astype(int)
            # i lied, this took me some time. but now, the parameters are
            # vertical to T_ and we are broadcasting like some crazy shit
            # pirate radio station
            return a + b_0 * np.sin(_T + phi) + b2 * np.sin(_T2 + phi2)
        except ValueError:
            # try with 3 parameters (happens during get_start_params
            a, b_0, phi = \
                np.array(trig_pars).reshape(-1, 3).T[:, :, np.newaxis]
            return a + b_0 * np.sin(_T + phi)


class SeasonalDist4d(SeasonalDist):
    def __init__(self, *args, **kwds):
        super(SeasonalDist4d, self).__init__(*args, **kwds)
        self.n_trig_pars = 4
        self.pre_start = [1]
    ...
    def trig2pars(self, trig_pars, _T=None):
        ...
        try:
            # funny how the obvious thing looks so complicated
            a, b_0, phi, a2 = \
                np.array(trig_pars).reshape(-1, 4).T[:, :, np.newaxis]
            # i lied, this took me some time. but now, the parameters are
            # vertical to T_ and we are broadcasting like some crazy shit
            # pirate radio station
            return a - b_0 * np.cos(_T + phi) * (a2 - np.cos(_T + phi))
        except ValueError:
            # try with 3 parameters (happens during get_start_params
            a, b_0, phi = \
                np.array(trig_pars).reshape(-1, 3).T[:, :, np.newaxis]
...
class SeasonalDist5d(SeasonalDist):
    def __init__(self, *args, **kwds):
        super(SeasonalDist5d, self).__init__(*args, **kwds)
        self.n_trig_pars = 5
        self.pre_start = [1, 0]
    ...
    def trig2pars(self, trig_pars, _T=None):
        ...
        try:
            # funny how the obvious thing looks so complicated
            a, b_0, phi, a2, b2 = \
                np.array(trig_pars).reshape(-1, 5).T[:, :, np.newaxis]
            # i lied, this took me some time. but now, the parameters are
            # vertical to T_ and we are broadcasting like some crazy shit
            # pirate radio station
            return a + b_0 * np.sin(_T + phi) * (a2 + b2 * np.sin(_T + phi))
        except ValueError:
            # try with 3 parameters (call happens during get_start_params)
            a, b_0, phi = \
                np.array(trig_pars).reshape(-1, 3).T[:, :, np.newaxis]
            return a + b_0 * np.sin(_T + phi)


class SeasonalDistPar(SeasonalDist):
    def __init__(self, *args, **kwds):
        super(SeasonalDistPar, self).__init__(*args, **kwds)
        self.n_trig_pars = 4
        self.pre_start = [1]
    ...
    def trig2pars(self, trig_pars, _T=None):
        ...
        try:
            ...
            # vertical to T_ and we are broadcasting like some crazy shit
            # pirate radio station
            return a + b_0 * (1 + np.sin(_T + phi)) ** c
        except ValueError:
            # try with 3 parameters (call happens during get_start_params)
            a, b_0, phi = \
                np.array(trig_pars).reshape(-1, 3).T[:, :, np.newaxis]
