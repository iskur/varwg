...
data_dir = os.path.join(os.path.dirname(vg.core.__file__),
                        "tests", "data")
...
class TestVG(npt.TestCase):
    ...
    def setUp(self):
        ...
        self.sim = np.squeeze(np.array([[met[var_name]]
                                        for var_name in var_names]))
        self.data_dir = tempfile.mkdtemp("vg_temporary_test")
        self.met_vg = vg.VG(var_names, refit="all", data_dir=self.data_dir,
    ...
    def test_multi_prim(self):
        ...
        rh_signal, _ = \
            self.met_vg.random_dryness(duration_min=7, duration_max=14)
        self.met_vg.simulate(primary_var=("theta", "rh"),
    ...
    def test_resample(self):
        ...
        kwds = dict()
        res_dict_nocy = vg.my.ADict(recalibrate=True,
                                    n_candidates=20)
        ...
        for res_dict in (res_dict_nocy, res_dict_cy):
            ...
            simt, sim = met_vg.simulate(mean_arrival=mean_arrival,
    ...
    def test_theta_incr(self):
        ...
        simt, sim = met_vg.simulate(theta_incr=theta_incr)
        npt.assert_almost_equal(sim[0].mean() - data_mean, theta_incr,
                                decimal=1)
        simt, sim = met_vg.simulate(theta_incr=theta_incr,
                                    mean_arrival=mean_arrival,
                                    disturbance_std=disturbance_std)
