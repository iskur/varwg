...
class Test(TestCase):
    ...
    def test_Monty(self):
        ...
        with monty.Monty(2, conf=vg.config_template) as mon:
            ...
            mon.run(n_realizations, use_seed=True,
                    sim_kwds=sim_kwds, dis_kwds=dis_kwds,
        ...
        for ri in range(n_realizations):
            seq = pd.read_csv(outfilepath_seq % ri, parse_dates=True,
                              index_col=0)
            par = pd.read_csv(outfilepath_par % ri, parse_dates=True,
                              index_col=0)
