    ...
    def decorate(func):
        ...
        if type(ddof) == int:
            wrapper.__doc__ = (wrapper.__doc__ +
                               _pars_descr_ddof.format(ddof=ddof))
        return wrapper
...
def _convolve(data, window_len=10, window_function=np.hanning,
              periodic=False, l_value=None, r_value=None, ddof=0,
              loo=False, no_future=False, **kwds):
    ...
    if loo:
        # leave one out
        smooth_vector[old_div(window_len, 2)] = 0

    if no_future:
        # no future no future for you
        # god save the queen
        smooth_vector[int(np.ceil(window_len / 2.))] = 0
    ...
    smoothed = np.convolve(smooth_vector / (smooth_vector.sum() - ddof),
                           data_window, mode='same')

    return smoothed[window_len:-window_len]


@_docstring_pars(ddof=0)
def smooth(data, window_len=10, window_function=np.hanning, periodic=False,
           l_value=None, r_value=None, loo=False, no_future=False, **kwds):
...
def variance(data, window_len=10, window_function=np.hanning, periodic=False,
             l_value=None, r_value=None, loo=False, no_future=False, **kwds):
    """Moving variance."""
    means = smooth(**locals())
...
@_docstring_pars(ddof=1)
def std(data, window_len=10, window_function=np.hanning, periodic=False,
        l_value=None, r_value=None, loo=False, no_future=False, **kwds):
...
def skew(data, window_len=10, window_function=np.hanning, periodic=False,
         l_value=None, r_value=None, loo=False, no_future=False):
    """Moving skew."""
    means = smooth(**locals())
    variances = variance(**locals())
    data = (old_div((data.copy() - means), variances ** .5)) ** 3
    return _convolve(ddof=2, **locals())


@_docstring_pars
def _roll_data(data, window_len=10, periodic=False, l_value=None, r_value=None,
               loo=False, no_future=False, **kwds):
...
@_docstring_pars
def min(data, window_len=10, periodic=False, l_value=None, r_value=None,
        loo=False, no_future=False, **kwds):
    """Moving minimum."""
...
@_docstring_pars
def max(data, window_len=10, periodic=False, l_value=None, r_value=None,
        loo=False, no_future=False, **kwds):
    """Moving maximum."""
...
@_docstring_pars
def maxdiff(data, window_len=10, periodic=False, l_value=None,
            r_value=None, loo=False, no_future=False, **kwds):
...
@_docstring_pars
def mindiff(data, window_len=10, periodic=False, l_value=None,
            r_value=None, loo=False, no_future=False, **kwds):
...
@_docstring_pars
def percentile(data, perc, window_len=10, periodic=False, l_value=None,
               r_value=None, loo=False, no_future=False):
    """Moving percentile."""
...
@_docstring_pars
def median(data, window_len=10, periodic=False, l_value=None, r_value=None,
           loo=False, no_future=False, **kwds):
    """Moving median."""
    return percentile(data, 50, window_len, periodic, l_value, r_value, loo,
                      no_future=no_future)
...
def crosscorr(data1, data2, lag=1, window_len=100):
    ...
    data1_rolled = \
        np.array([np.roll(data1, window_len - 1 - shift)[:window_len]
                 for shift in range(len(data1))])
    data2_rolled = \
        np.array([np.roll(data2, window_len - 1 - shift)[:window_len]
    ...
    return old_div(np.mean(covs, axis=1), (np.std(data1_rolled, ddof=1, axis=1) *
...
def autocorr(data, lag=1, window_len=100):
    ...
    # with all windows
    data_rolled = np.array([np.roll(data, window_len - 1 - shift)[:window_len]
                            for shift in range(len(data))])
    means = data_rolled.mean(axis=1)[:, np.newaxis]
    covs = (data_rolled[:, :-lag] - means) * (data_rolled[:, lag:] - means)
    return old_div(np.mean(covs, axis=1), np.var(data_rolled, ddof=1, axis=1))
